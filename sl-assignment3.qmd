...
library(keras)       # For neural networks if available
...
The target variable will be derived from **charges** (medical costs billed), which we'll convert to a binary outcome (high/low) for classification.
...
# Create a binary target variable based on charges
# Using median as the threshold for simplicity
median_charge <- median(insurance_data$charges)
insurance_data$charges_class <- factor(ifelse(insurance_data$charges > median_charge, "high", "low"),
                                      levels = c("low", "high"))
```
...
For the classification task, we've converted the continuous "charges" variable into a binary outcome:
- "high" = charges above the median (`r scales::dollar(median_charge)`)
- "low" = charges below or equal to the median

This transformation allows us to apply classification algorithms to predict whether a patient will have high or low insurance costs.
...
class_table <- table(insurance_data$charges_class)
...
p1 <- ggplot2::ggplot(insurance_data, ggplot2::aes(x = charges_class, y = age, fill = charges_class)) +
...
p2 <- ggplot2::ggplot(insurance_data, ggplot2::aes(x = charges_class, y = bmi, fill = charges_class)) +
  ggplot2::geom_boxplot() +
  ggplot2::theme_minimal() +
...
p3 <- ggplot2::ggplot(insurance_data, ggplot2::aes(x = charges_class, fill = smoker)) +
...
p4 <- ggplot2::ggplot(insurance_data, ggplot2::aes(x = charges_class, fill = sex)) +
  ggplot2::geom_bar(position = "fill") +
  ggplot2::theme_minimal() +
  ggplot2::labs(x = "Charges Class", y = "Proportion") +
  ggplot2::scale_fill_brewer(palette = "Set2") +
...
ggplot2::ggplot(insurance_data, ggplot2::aes(x = bmi, y = charges, color = smoker, shape = charges_class)) +
...
# Remove original charges column from training set
model_data <- insurance_data %>%
  dplyr::select(-charges)

# Split data into features and target
X <- model_data %>% dplyr::select(-charges_class)
y <- model_data$charges_class
...
train_data <- cbind(X_train, charges_class = y_train)
